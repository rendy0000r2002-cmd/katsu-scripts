"""檢查 deployed client bundle 是否含有新的 onChange / setActiveSchedule(null) 邏輯"""
...
# 不用 PTY，避免 sudo prompt 干擾
cmd = (
    f"echo ***REDACTED-NAS-PASS*** | sudo -S -p '' /usr/local/bin/docker exec katsu-web sh -c "
    f"\"echo {b64} | base64 -d > /tmp/probe.js && node /tmp/probe.js\" 2>&1"
)
