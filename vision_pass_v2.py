...
判斷原則：
- place 只能是「相機/無人機實際所在、這支影片真正在拍攝的地點」。
  畫面遠方天際線、地平線、背景出現的地標（即使很有名）**不算拍攝地**。
  例：空拍畫面遠處可見台北101，但無人機其實在新北永和上空拍永和水岸 → place 不可填台北101，應回 null 或填永和當地地標。
- 空拍尤其注意：高空能看到很遠的著名建築，但要判斷的是「飛行/取景的所在區域」，不是遠方天際線那棟最顯眼的建築。
- 看到明顯招牌/Logo/建築特徵/地標，且確實是拍攝主體所在地 → place 填具體地名 + confidence 0.7~1.0
- 只看得出大致氛圍但無具體線索，或唯一可辨識的地標只是遠景/背景 → place=null
- 偏向保守：寧可 null 也不要瞎猜
- place 用簡稱（不加「市/區」字）
- 只回 JSON，不要其他文字"""
